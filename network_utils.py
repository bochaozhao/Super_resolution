#modified from https://github.com/togheppi/pytorch-super-resolution-model-collection/blob/master/utils.py
...
def save_img(img, img_num, save_dir='', is_training=False):
    ...
    if list(img.shape)[0] == 3:
        save_img = img*255.0
        save_img = save_img.clamp(0, 255).detach().numpy().transpose(1, 2, 0).astype(np.uint8)
        # img = (((img - img.min()) * 255) / (img.max() - img.min())).numpy().transpose(1, 2, 0).astype(np.uint8)
    else:
        save_img = img.squeeze().clamp(0, 1).detach().numpy()
        save_img=transforms_3d.rescale(save_img,original_scale=(0,1),new_scale=(0,255))
    # save img
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    if is_training:
...
def plot_test_result(imgs, metrics, img_num, save_dir='', is_training=False, show_label=True, show=False,index='sc'):
    ...
    # axes.axis('off')
    for i, (ax, img, metric) in enumerate(zip(axes.flatten(), imgs, metrics)):
        ax.axis('off')
        ...
        if show_label:
            ...
            if i == 0:
                ...
                ax.set_xlabel('Bicubic (%s: %.2f)' % (index,metric))
            elif i == 3:
                ax.set_xlabel('SR image (%s: %.2f)' % (index,metric))
...
def PSNR(pred, gt):
    ...
    diff = pred - gt
    mse = np.mean(diff.detach().numpy() ** 2)
    if mse == 0:
        return 100
...
#Structural comparison term in SSIM
def SC(pred, gt):
    pred = pred.clamp(0, 1)
    # pred = (pred - pred.min()) / (pred.max() - pred.min())
    pred=pred.detach().numpy().squeeze()
    ...
    return misc.structural_similarity(pred,gt,data_range=1.0)



#Normalization function
def norm(imgs, vgg=False):
